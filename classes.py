...
class Sprite(pygame.sprite.Sprite):
    def load_image(self, filepath, x, y, size_x=None, size_y=None):
...
class Level:
    def __init__(self):
        self.raw_map = []
        self.camera = None
        self.image = None  # спрайты стен, пола и тп без "эффектов" объеденины в одну картинку
        self.rect = None
        self.mask = None
        self.walls_cl = []  # стены с колизией на границе комнаты и пространства стены

        self.sprites = self.load_sprites(block_width, block_height)
        self.wall_sp = self.sprites.get('img_wall')  # вместо self.sprites.get('img_wall') в gen будем дергать это, так оптимальнее

    def load_sprites(self, size_x, size_y):
        # загрузка всех спрайтов для будующего использования в словарь
        stone_wall_sp = Sprite()
        stone_wall_sp.load_image("test_wall_block.png", size_x, size_y)
        stone_wall = stone_wall_sp.image
        sp = {
            'img_wall': stone_wall
        }
        return sp
    ...
    def gen(self, mappath):
        ...
        self.image = pygame.surface.Surface((len(self.raw_map) * block_width, len(self.raw_map[0]) * block_height))
        test = 0
        for i, a in enumerate(self.raw_map):
            for j, b in enumerate(a):
                if self.raw_map[i][j] == '0':
                    self.image.blit(self.wall_sp, (block_width * i, block_height * j))
                    #self.stones.append(Block(self.size * i, self.size * j, 0, self.size))
                    print(test)
                    test += 1
                elif self.raw_map[i][j] == '3':
                    print('rar')
